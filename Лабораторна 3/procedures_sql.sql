...
/*ТРИГЕР*/
CREATE OR REPLACE FUNCTION clean_category_on_new_product() RETURNS TRIGGER AS $$
BEGIN
    -- видалити продукти з тієї ж категорії, якщо вони не використовуються у order_details
    DELETE FROM Products
    WHERE category_id = NEW.category_id
      AND id != NEW.id
      AND id NOT IN (SELECT product_id FROM order_details);

    RETURN NEW;
END;
$$ LANGUAGE plpgsql;

CREATE TRIGGER trigger_clean_category_on_new_product
AFTER INSERT ON Products
FOR EACH ROW
EXECUTE FUNCTION clean_category_on_new_product();
*/
